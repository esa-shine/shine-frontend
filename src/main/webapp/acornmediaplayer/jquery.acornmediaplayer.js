...
(function($) {
	$.fn.acornMediaPlayer = function(options, videoDuration, timer) {
		/*
		 * Define default plugin options
		 */
        // var timer = new Timer();
        var fileName= getParameterByName("name")+'.webm';

        var duration;
        var vDur;


            $.get("/media/search/findByName?name="+fileName, function(data) {
                console.log("retrieve data", data);
                duration= data;
                vDur=duration.duration/1000;
            });
		...
		var acornPlayer = function() {
			...
			*/
			...
			var startPlayback = function() {
                ...
                timer.start({precision: 'seconds', startValues: {seconds: 0}, target: {seconds: vDur}});
			};
			...
			var seekUpdate = function() {
				...
				var playbackSecond = timer.getTimeValues().seconds;
				var playbackMinute= timer.getTimeValues().minutes;
				var playbackHours= timer.getTimeValues().hours;
				var currenttime= playbackHours*3600+playbackMinute*60+playbackSecond;
// console.log('cccccc',currenttime);
				// acorn.$timer.text(timeFormat(currenttime));
				acorn.$timer.text(timer.getTimeValues().toStringMS());
...
// 				acorn.$self.prop('currentTime', seekLocation);
				...
				console.log('seekto',seekLocation);
				timer.stop();

				timer.start({precision: 'seconds', startValues: {seconds: seekLocation}, target: {seconds: vDur}});
				blurCaptionBtn();
			};
			...
			var updateCaption = function() {
				...
				// var now = acorn.$self.prop('currentTime'),
					var now = timer.getTimeValues().toStringMS(),
					text = '',
					i,
			...
			var initCaption = function() {
				...
				if(acorn.$track.length>1) {
					...
					var changeCaption = function() {
						// get the original <track> "janus" attribute from the custom "data-url" attribute of the radio input
